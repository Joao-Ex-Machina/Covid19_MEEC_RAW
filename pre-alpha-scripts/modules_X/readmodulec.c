...
country_list* readDATfile(char* _filename,country_list* heade){
    country_list *header = heade, *aux = NULL, *aux2 = NULL;
    FILE *fp = fopen(_filename,"rb");
    char country[30];
    char country_code[4];
    char continent[15];
    long population;
    long death_Weekly_count,long death_cumulative_count,long infected_Weekly_count,long infected_cumulative_count,long year,long week;
    double death_rate_14_day,double infected_rate_14_day;
    fread
    header->next=create_new_country();
    aux=header->next;
    aux->week_pointer=create_new_weekDAT();
    while(feof(fp) == 0){
