...
country_list* readDATfile(char* _filename,country_list* heade){
    ...
    header->next=create_new_country(country,country_code,continent, population);
    aux=header->next;
    ...
    while(feof(fp) == 0){
        ...
        if (strcmp(country, aux->country)==0){
            ...
            aux=aux->next;
            aux_week=aux->week_pointer;
            aux_week=create_new_weekDAT(death_Weekly_count, death_cumulative_count, infected_Weekly_count, infected_cumulative_count, year, week, death_rate_14_day,infected_rate_14_day);
        }
    }
    fclose(fp);
    return header;
