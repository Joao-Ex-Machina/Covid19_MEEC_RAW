...
country_list *CumulativeValue_sorter(country_list *header, char indicator[4] , long aux_year, long aux_week){
    ...
    if ((header!=NULL) && (header->next !=NULL)){
        while(switcher_aux){
            ...
            while((nxt_right_week->week != aux_week || nxt_right_week->year !=aux_year) && nxt_right_week->next!=NULL){
                nxt_right_week=nxt_right_week->next;
            }
            while((right_week->week != aux_week || right_week->year !=aux_year)&& right_week->next!=NULL ){
                right_week=right_week->next;
            }
            if(right_week->next==NULL ||nxt_right_week->next==NULL)
                break_flag=TRUE;
